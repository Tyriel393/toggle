...
**Phase F — integration, orientation, mobile (verified in the live product first — `docs/integration-map.md`)**
- [x] F0 **Checked before building.** Capacity inputs traced in the real app: Working hours (per-member, **unset — reads `-`** while Workload still asserts 40h/week), Time off (**$2/user/mo add-on**, "automatically reflected in team capacity"), calendar events (skippable onboarding step), dated estimates. **The capacity our feature reasons about is a default nobody set** — reachable on day one, and exactly the "capacity distrusted" kill criterion.
- [x] F1 **Capacity provenance** — the week strip cites `8h/day from your working hours` with an inline explainer naming the unset default, time off and calendar as the other inputs. A number whose source you can see is a number you can argue with.
- [x] F2 **Guided tour** — 3 anchored coach marks with spotlight ring, keyboard-navigable (`→`/`Enter`/`←`/`Esc`), skippable, replayable from the demo pill. A cold evaluator now knows the scenario before touching anything.
- [x] F3 **`/setup` — the week-one setup step.** Frames dates+estimates by what they buy: *"Toggl can tell you when a job running long puts another client's deadline at risk… add them now and Toggl will catch the collision this week."* **This is the product answer to the eligibility risk**, not demo polish — it is how a W0 user comes to have dated, estimated work at all. Emits `setup_work_added` / `setup_completed` / `setup_skipped`.
- [x] F4 **Mobile settled by evidence, not opinion.** Toggl 2.0 ships its own gate — *"Toggl 2.0 works better on bigger screens"* (found live; present in the DOM at 1703px with a zero-size box, shown below the breakpoint). The prototype mirrors it below 768px and names the honest split: the **ask** belongs on a phone as a notification with `Done / +1h / Not sure` actions; the **replan** is desktop work. Building a responsive Toggl 2.0 would diverge from the real product.
- [x] F5 Bug caught in verification: `/setup`'s add-handler used a stale closure, so three fast clicks registered one. Fixed with a functional update; 3-of-3 confirmed.
